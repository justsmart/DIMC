...
def train_DIC(mul_X, mul_X_val, WE,WE_val,yv_label, device,args):
    ...
    for epoch in range(int(args.maxiter)):
        ...
        if epoch > 100 and ( (best_value_result[4]-value_result[4]>0.03) or
                best_value_result[4] < args.min_AP or (abs(total_loss_last - total_loss) < 1e-5)):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--min_AP', default=0.1, type=float)
    parser.add_argument('--tol', default=1e-7, type=float)
    args = parser.parse_args()

    args.cuda = torch.cuda.is_available()
    print("use cuda: {}".format(args.cuda))
    file_path = 'sup-DIMC-' + args.dataset + '_BS_' + str(args.batch_size) + '_VMR_' + str(
    ...
    pre_alpha = [0.1] # 
    pre_beta = [0] #[1e-5,1e-3,1e-1]
    pre_gamma = [0]  # [1e0,1e-1,1e-2,1e-3,1e-4]
    best_AUC_me = 0
    best_AUC_mac = 0
    best_AP = 0
